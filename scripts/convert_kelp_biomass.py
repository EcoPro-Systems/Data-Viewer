...
def main():
    ...
    # read data arrays
    data_lat = var_lat.ReadAsArray(buffer_datatype = gdal.ExtendedDataType.Create(gdal.GDT_Float64))
    data_lon = var_lon.ReadAsArray(buffer_datatype = gdal.ExtendedDataType.Create(gdal.GDT_Float64))
    data_time = var_time.ReadAsArray(buffer_datatype = gdal.ExtendedDataType.Create(gdal.GDT_Float64))
    data_targets = [(var_t[0], var_t[1].ReadAsArray(buffer_datatype = gdal.ExtendedDataType.Create(gdal.GDT_Float64))) for var_t in var_targets]
    ...
    for idx in tqdm(range(data_time.shape[0])):
        ...
        data_frame['geometry'] = data_frame.apply(lambda x: Point((x.longitude, x.latitude)), axis=1)
